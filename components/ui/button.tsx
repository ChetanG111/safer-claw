...
const buttonVariants = cva(
  "[&_svg]:-mx-0.5 relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-lg border font-medium text-base outline-none transition-shadow before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    ...
    variants: {
      size: {
        default: 'h-11 px-[calc(--spacing(4)-1px)] sm:h-10',
        icon: 'size-11 sm:size-10',
        'icon-lg': 'size-12 sm:size-11',
        'icon-sm': 'size-10 sm:size-9',
        'icon-xl':
          "size-13 sm:size-12 [&_svg:not([class*='size-'])]:size-5 sm:[&_svg:not([class*='size-'])]:size-4.5",
        'icon-xs':
          "size-9 rounded-md before:rounded-[calc(var(--radius-md)-1px)] sm:size-8 not-in-data-[slot=input-group]:[&_svg:not([class*='size-'])]:size-4 sm:not-in-data-[slot=input-group]:[&_svg:not([class*='size-'])]:size-3.5",
        lg: 'h-12 px-[calc(--spacing(4.5)-1px)] sm:h-11',
        sm: 'h-10 gap-1.5 px-[calc(--spacing(3)-1px)] sm:h-9',
        xl: "h-13 px-[calc(--spacing(5)-1px)] text-lg sm:h-12 sm:text-base [&_svg:not([class*='size-'])]:size-5 sm:[&_svg:not([class*='size-'])]:size-4.5",
        xs: "h-9 gap-1 rounded-md px-[calc(--spacing(2.5)-1px)] text-sm before:rounded-[calc(var(--radius-md)-1px)] sm:h-8 sm:text-xs [&_svg:not([class*='size-'])]:size-4 sm:[&_svg:not([class*='size-'])]:size-3.5",
      },
      variant: {
        default:
